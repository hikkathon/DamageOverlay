...
namespace WpfAppDPO.Models
{
    class MemoryEditor
    {
        ...
        {
            // Damage
            Int32 dmgAddr = BA + 0x02C3B35C; // 1 базовый адрес 2 смещение
            Int32[] dmgOffsets = { 0x38, 0x204, 0x114, 0x0, 0x30, 0x148, 0x24 }; // 0x94(old) + 0x8 = 0x9C(new)
            Damage = BitConverter.ToInt32(ReadMemory((IntPtr)ReadPointer(dmgAddr, dmgOffsets), (uint)4), 0);

            // Blocked
            Int32 blckAddr = BA + 0x02C3B35C; // 1 базовый адрес 2 смещение
            Int32[] blckOffsets = { 0x38, 0x204, 0x114, 0x0, 0x30, 0x148, 0x28 };
